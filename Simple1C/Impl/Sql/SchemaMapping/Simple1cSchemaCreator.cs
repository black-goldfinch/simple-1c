...
namespace Simple1C.Impl.Sql.SchemaMapping
{
    internal class Simple1CSchemaCreator
    {
        ...
        private void RemoveHelperFunctions()
        {
            var sql = @"
                DROP FUNCTION IF EXISTS simple1c.to_type_name(bytea);
                DROP FUNCTION IF EXISTS simple1c.to_guid(bytea);
                DROP FUNCTION simple1c.date_from_guid(varchar(36));
            ";
